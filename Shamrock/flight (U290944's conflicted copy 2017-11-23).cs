...
namespace Shamrock
{
    public class flight
    {
        ...
        {
            int ret = 0;
            if (CountPlayer() > 1 && ContainsPlayer(P1.name) && ContainsPlayer(P2.name) & GetHowManyTimeInSameTeam(P1, P2) == 0 )
                ++ret;
            return ret;
        }
        ...
        {
            int ret = 0;
            if (CountPlayer() > 1 && ContainsPlayer(P1.name) && ContainsPlayer(P2.name))
                ++ret;
            return ret;
        }
    ...
    {
        //List<string> histPlayers = new List<string>();
        ...
        //{
        //    if(!histPlayers.Contains(playerName))
        //    {
        //        histPlayers.Add(playerName);
        //    }
        //    refreshPlayerPairsKey();
        //}
        //void refreshPlayerPairsKey()
        //{
        //    foreach (string p1 in histPlayers)
        //    {
        //        foreach (string p2 in histPlayers)
        //        {
        //            string key = getPairsKey(p1, p2);
        //            if (!playersPairsKey.Contains(key))
        //                playersPairsKey.Add(key);
        //        }
        //    }
        //}
                ...
                foreach (flight ctFlight in ctDay.flights.Values)
                {
                    foreach (match match in ctFlight.matchs.Values)
                    {
                        if (ctDay.matchScores.matchResults.ContainsKey(match.name))
                        {
                            ...
                            throw new Exception(String.Format("Could not find matchScore for this match on day {0} (y{2}): {1}", ctDay.nr, match.name, ctC.year));
                    }
                }
            }
        }
